...
export class ChildMovingInformationComponent extends Base implements OnInit {
  ...
  get requestSchoolInfo() {
    return (
      this.isCanadianFromCountry ||
      this.isCanadianFromProv ||
      this.isCanadianNotBC ||
      this.isResidentFromCountry ||
      this.isResidentFromProv ||
  ...
  get requestPermMoveInfo() {
    return (
      this.isCanadianFromProv ||
      this.isCanadianFromCountry ||
      (this.isCanadianNotBC && this.person.livedInBCSinceBirth) ||
      this.isResidentFromProv ||
      this.isResidentFromCountry ||
      this.isResidentNotBC
    );
  ...
  get showRecentMove() {
    return (
      ...
      (this.person.livedInBCSinceBirth === false && this.isChild)
    );
  }
  ...
  get arrivalToBCEndRange() {
    if (this.departureDateDuring12MonthsDate) {
      return isBefore(
        this.departureDateDuring12MonthsDate,
        this.arrivalToBCDate
      )
        ? this.departureDateDuring12MonthsDate
        : this.dateToday;
    }
  ...
  get departureDateDuring12MonthsStartRange() {
    if (this.arrivalToBCDate && this.dob) {
      return isBefore(this.arrivalToBCDate, this.dob)
        ? this.dob
        : this.arrivalToBCDate;
  ...
  get studiesBeginDateStartRange() {
    if (
      ...
      isBefore(this.dob, this.studiesDepartureDate)
    ) {
  ...
  get studiesFinishedDateStartRange() {
    return this.studiesBeginDate > this.dateToday
      ? this.studiesBeginDate
      : this.dateToday;
  }
  ...
  get departureDate6MonthsErrorMessage(): ErrorMessage {
    ...
    if (
      this.returnDateDuring6MonthsDate <
      addDays(this.departureDateDuring6MonthsDate, 30)
    ) {
      return {
