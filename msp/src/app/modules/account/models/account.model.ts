...
class AccountChangeOptions {
  ...
  hasAllOptionsSelected(): boolean {
    return (
      this.personInfoUpdate &&
      this.dependentChange &&
      this.addressUpdate &&
      this.statusUpdate
    );
  }
  ...
  hasOnlyAddressSelected() {
    return (
      this.addressUpdate &&
      !(this.personInfoUpdate || this.dependentChange || this.statusUpdate)
    );
  }
...
class MspAccountApp implements ApplicationBase {
  ...
  public isMembersPresentWithStatus(
    ...
    return (
      personList.filter((person) => person.status === statusInCanada).length > 0
    );
  }
  public isAnyPRinPersonalInfoPage(): boolean {
    return this.isMembersPresentWithStatus(
      this.allPersonsInPI,
  ...
  get isUniquePhnsinDependents() {
    const allPhs: string[] = this.allPersonsInDep
      .filter((x) => x)
  ...
  get isUniquePhnsInPI() {
    const allPhs: string[] = this.allPersonsInPI
      .filter((x) => x)
      .map((x) => x.previous_phn)
      .filter((x) => x)
      .filter((x) => x.length >= 10);
  ...
  getallSpouses(): MspPerson[] {
    ...
    if (
      (this.accountChangeOptions.personInfoUpdate ||
        this.accountChangeOptions.statusUpdate) &&
      this.updatedSpouse
    ) {
